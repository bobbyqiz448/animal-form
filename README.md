...
* [`src/animal_form_native/`](src/animal_form_native/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/animal_form_native/core.cljs): contains the SPA entry point, `init`
...
5. Clone this repo and open a terminal in the `animal-form-native` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/animal_form_native/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns animal-form-native.example
  (:require [animal-form-native.config :as config])
